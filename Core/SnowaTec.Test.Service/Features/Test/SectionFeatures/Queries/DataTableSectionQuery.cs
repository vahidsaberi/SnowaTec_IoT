...
{
    public class DataTableSectionQuery : IRequest<Response<DTResult<Section>>>
    {
        public DTParameters Parameters { get; set; }

        public class DataTableSectionQueryHandler : IRequestHandler<DataTableSectionQuery, Response<DTResult<Section>>>
        {
            private readonly ITestDbContext _context;

            public DataTableSectionQueryHandler(ITestDbContext context)
            {
                _context = context;
            }

            public async Task<Response<DTResult<Section>>> Handle(DataTableSectionQuery request, CancellationToken cancellationToken)
            {
                try
                {
                    var allDataCount = _context.Sections.Count();
                    ...
                    {
                        var listOfFields = string.Join('+', _context.Sections.Select(x => nameof(x)));
                    ...
                    var modelList = _context.Sections.FromSqlRaw(query);
                    ...
                    if (modelList == null) return new Response<DTResult<Section>> { Succeeded = false, Errors = new List<string> { "اطلاعات مورد نظر پیدا نشد." } };

                    var data = new DTResult<Section>
                    {
                        data = result,
                    ...
                    return new Response<DTResult<Section>> { Data = data, Message = "بارگذاری اطلاعات با موفقیت انجام شد.", Succeeded = true };
                }
                catch (System.Exception ex)
                {
                    return new Response<DTResult<Section>> { Succeeded = false, Errors = new List<string> { ex.Message } };
                }
            }
        }
    }
